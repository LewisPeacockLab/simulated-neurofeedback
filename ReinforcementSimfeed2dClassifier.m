classdef ReinforcementSimfeed2dClassifier < handle
    properties
    % properties here
    N_training_samples = 140 % samples per class
    N_test_samples = 70 % samples per class
    N_class = 3
    fmri_data
    labels
    weights
    ...
    methods
        ...
        function sampleClassifier(self, brain)
            ...
            [self.weights, self.ix_eff, self.errTable_tr, self.errTable_te] = muclsfy_smlr(...
                ...
                'wdisp_mode', 'iter', 'nlearn', 300, 'mean_mode', 'none', 'scale_mode', 'none', 'amax', 1e8);
        end
        ...
        function mean_class_probs = testClassifierSim(self, brain)
            ...
            for orientation = 1:self.N_class
                for sample = 1:self.N_test_samples
                    all_orientation_probs = self.applyClassifier(brain.sampleVolume(10+60*(orientation-1)));
        ...
        function [problem_brain, problem_probs] = testClassifierNan(self, brain)
            for orientation = 1:self.N_class
                for sample = 1:self.N_test_samples
                    new_brain = brain.sampleNoisyVolume(10+60*(orientation-1));
        ...
        function [means, vars, maxes, mins] = testClassifierRaw(self, brain)
            ...
            for orientation = 1:self.N_class
                for sample = 1:self.N_test_samples
                    class_outs(orientation,:,sample) = self.applyClassifierRaw(brain.sampleNoisyVolume(10+60*(orientation-1)));
                end
            end
        ...
        function class_probs = applyClassifier(self, fmri_data)
            % [tmp, label_est] = max(fmri_data' * self.weights(1:400,:),[],2);
            eY = exp(fmri_data'*self.weights(1:400,:)); % Nsamp*Nclass
            class_probs = eY ./ repmat(sum(eY,2), [1, self.N_class]); % Nsamp*Nclass
        end

        function class_out = applyClassifierRaw(self, fmri_data)
            class_out = fmri_data'*self.weights(1:400,:);
        end
    end
